...
def init_net(net, cfg_init_net, cfg_pretrained, resume_net):
    if cfg_init_net and not resume_net:
        net.init_model(cfg_pretrained)
    else:
...
def image_forward(img, net, cuda, priors, detector, transform=None):
    w, h = img.shape[1], img.shape[0]
    scale = torch.Tensor([w, h, w, h])
    with torch.no_grad():
        x = img  # transform(img).unsqueeze(0)
        if cuda:
            x = x.cuda()
            scale = scale.cuda()
    out = net(x)
