...
class M2Det(nn.Module):
    def __init__(self, phase, size, num_classes, config=None):
        '''
        M2Det: Multi-level Multi-scale single-shot object Detector
        '''
        ...
        self.base_out = [22, 34]  # [22,34] for vgg, [2,4] or [3,4] for res families
        self.planes = 256
        self.num_levels = 8
        self.num_scales = 6
        self.sfam = False
...
def build_net(num_classes, phase='train', size=320, config=None):
    ...
    return M2Det(phase, size, num_classes, config)
