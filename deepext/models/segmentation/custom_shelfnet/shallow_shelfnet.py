...
__all__ = ['ShallowShelfNet']


# TODO
class ShallowShelfNet(SegmentationModel):
    def __init__(self, n_classes: int, out_size: Tuple[int, int], in_channels=3, lr=1e-3, loss_type="ce",
        ...
        self._model: nn.Module = try_cuda(
            ShallowShelfNetModel(n_classes=n_classes, out_size=out_size, in_channels=in_channels, backbone=backbone))
...
class ShallowShelfNetModel(nn.Module):
    def __init__(self, n_classes: int, out_size: Tuple[int, int], in_channels=3, backbone="resnet18"):
        ...
        self._multi_scale_backbone = ResNetMultiScaleBackBoneWithFirstLayer(resnet_type=backbone)
